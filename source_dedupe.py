...
def nasa_search(q):
    return requests.get('https://images-api.nasa.gov/search',params={'q':q,'media_type':'video','page_size':70},timeout=30).json()['collection'].get('items',[])

def nasa_assets(nid):
    return [x.get('href','') for x in requests.get('https://images-api.nasa.gov/asset/'+quote(nid,safe=''),timeout=30).json()['collection'].get('items',[])]
...
# Distinct means distinct underlying production, not merely a clean/codec/resolution variant.
used=set(); dup=[]
for i,x in enumerate(manifest):
    if x.get('status')!='SELECTED': continue
    f=family(x.get('title') or x.get('asset_identity'))
    x['source_family']=f
    if f in used: dup.append(i)
    else: used.add(f)

replacement_queries={
  3:['astronaut ultrasound human research ISS','astronaut physiology experiment ISS','astronaut medical research space station'],
  4:['astronaut human research body ISS','astronaut medical assessment spaceflight','astronaut exercise human physiology ISS','astronaut health experiment ISS']
}
...
for idx in dup:
    ...
    for q in replacement_queries.get(scene,[]):
        ...
        for it in items:
            ...
            text=(' '.join([title,desc,kws])).lower(); fam=family(title or nid)
            ...
            for url in urls:
                ...
                found=dict(old)
                found.update({'source_type':'NASA','asset_identity':nid,'title':title,'description_excerpt':re.sub(r'\s+',' ',desc)[:300],'direct_download_url':url,'source':'NASA Image and Video Library','license':'NASA U.S. Government media','width':pr[0],'height':pr[1],'duration':round(pr[2],2),'native_4k':True,'source_family':fam,'dedupe_replacement':True})
                break
            if found: break
        if found: break
    if found:
        manifest[idx]=found; used.add(found['source_family'])
    else:
        old['status']='DUPLICATE_SOURCE_FAMILY_NO_REPLACEMENT'

selected=[x for x in manifest if x.get('status')=='SELECTED']
families={family(x.get('title') or x.get('asset_identity')) for x in selected}
gate=json.loads((OUT/'source_gate.json').read_text(encoding='utf-8'))
gate['unique_source_families']=len(families)
gate['minimum_unique_source_families']=4
gate['duplicate_variant_policy']='CLEAN_CODEC_RESOLUTION_VARIANTS_COUNT_AS_ONE_SOURCE'
gate['ready']=len(selected)==len(manifest) and len(families)>=4 and all(x.get('native_4k') for x in selected)
...
print(json.dumps({'ready':gate['ready'],'selected':len(selected),'unique_source_families':len(families)}))
