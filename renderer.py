...
def safe_url(url):
    """Percent-encode Unicode/path characters while preserving URL structure and query separators."""
...
def dl(url,path,max_attempts=5):
    ...
    for attempt in range(1,max_attempts+1):
        ...
        try:
            ...
            tmp.replace(path)
            print(f'Downloaded {path.name}: {path.stat().st_size/1024/1024:.1f} MiB',flush=True); return
        except urllib.error.HTTPError as e:
            last=e; retry_after=e.headers.get('Retry-After') if e.headers else None
            if e.code not in (429,500,502,503,504): raise
            delay=min(120,int(retry_after)) if retry_after and retry_after.isdigit() else waits[min(attempt-1,len(waits)-1)]
            print(f'Download throttled/error HTTP {e.code}; attempt {attempt}/{max_attempts}, waiting {delay}s',flush=True)
        except Exception as e:
            last=e; delay=waits[min(attempt-1,len(waits)-1)]
...
for i,b in enumerate(plan['beats'],1):
    ...
    vf=("scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,eq=contrast=1.04:saturation=1.04,"
        f"drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text='{cap}':fontcolor=white:fontsize=66:borderw=6:bordercolor=black@0.85:x=(w-text_w)/2:y=h*0.68")
    run(['ffmpeg','-y','-ss',f'{start:.2f}','-i',raw,'-t',f'{dur:.2f}','-an','-vf',vf,'-r','30','-c:v','libx264','-preset','veryfast','-crf','19','-pix_fmt','yuv420p',out]); clips.append(out)
